...
select role,name into v_role,v_reviewer_name from public.user_roles where user_id=nullif(auth.uid(),'')::uuid and is_active=true order by case role when 'chief' then 1 when 'teacher' then 2 when 'president' then 3 else 99 end limit 1;
...
begin
if auth.uid() is null then raise exception '로그인이 필요합니다.'; end if;
select role into v_role from public.user_roles where user_id=nullif(auth.uid(),'')::uuid and is_active=true order by case role when 'chief' then 1 when 'teacher' then 2 when 'president' then 3 else 99 end limit 1;
...
if v_author is null then raise exception '보고서를 찾을 수 없습니다.'; end if;
v_is_author:=v_author=nullif(auth.uid(),'')::uuid; v_is_chief:=public.has_any_active_role(array['chief']);
