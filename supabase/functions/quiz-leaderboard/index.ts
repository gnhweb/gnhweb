    ...
    if (req.method === 'GET') {
      ...
      if (effectiveUserId) {
        ...
        return new Response(JSON.stringify({
          ...
          monthly,
          sessions: userRows.sort((a: { created_at: string }, b: { created_at: string }) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()),
      ...
      for (const row of (allScores || [])) {
        ...
        if (!existing) {
          ...
          if ((row.score || 0) > existing.best_score) existing.best_score = row.score || 0;
      ...
      for (const u of userCumulative.values()) {
        ...
        if (!clubMap2.has(cn)) clubMap2.set(cn, { club_name: cn, total_score: 0, member_count: 0, total_correct: 0, total_questions: 0 });
      ...
      const clubRanking = [...clubMap2.values()].map(c => ({
        ...c,
      ...
      return new Response(JSON.stringify({ scores: uniqueScores, clubRanking, topClub, topPlayer, monthly }), { headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' } });
    }

    if (req.method === 'POST') {
      const body: QuizScore = await req.json();
      if (!body.user_id || !body.nickname || !body.club_name || body.score === undefined) {
        return new Response(JSON.stringify({ error: '필수 항목이 누락되었습니다.' }), { status: 400, headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' } });
      }
      const { data: inserted, error: insertError } = await supabase.from('quiz_scores').insert({
      ...
      const { data: userAll } = await supabase.from('quiz_scores').select('score, correct_count, total_questions').eq('user_id', body.user_id);
      const cumulative = (userAll || []).reduce((acc, r) => ({
        ...
        accuracy: acc.total_questions + r.total_questions > 0 ? Math.round(((acc.total_correct + r.correct_count) / (acc.total_questions + r.total_questions)) * 100) : 0,
      ...
      return new Response(JSON.stringify({ success: true, session: inserted, cumulative, streak: streak || undefined }), { headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' } });
    }

    if (req.method === 'DELETE') {
      const authHeader = req.headers.get('Authorization');
      if (!authHeader) return new Response(JSON.stringify({ error: '인증 정보가 없습니다.' }), { status: 401, headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' } });
      const token = authHeader.replace('Bearer ', '');
      const { data: { user } } = await supabase.auth.getUser(token);
      if (!user) return new Response(JSON.stringify({ error: '인증에 실패했습니다.' }), { status: 401, headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' } });
      const { data: roleRow } = await supabase.from('user_roles').select('role').eq('user_id', user.id).maybeSingle();
      const { data: extraRoles } = await supabase.from('user_role_assignments').select('role').eq('user_id', user.id);
      const allRoles = [roleRow?.role, ...(extraRoles || []).map((r: { role: string }) => r.role)].filter(Boolean);
      const isChief = allRoles.includes('chief');
      if (!isChief) return new Response(JSON.stringify({ error: '부장만 리더보드를 초기화할 수 있습니다.' }), { status: 403, headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' } });
      const { error: deleteError } = await supabase.from('quiz_scores').delete().not('id', 'is', null);
