  ...
  try {
    ...
    if (NIM_API_KEY) {
      try {
        ...
        const aiResult = await callNVIDIA(prompt);
        const parsed = parseInsightResponse(aiResult, currentMeetingId as string);
        return new Response(JSON.stringify(parsed), {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      } catch (nimErr) {
        console.error("NIM API error, using fallback:", nimErr);
      }
    }
...
async function callNVIDIA(prompt: string): Promise<string> {
  ...
  logNvidiaUsage("meeting-insight-ai", "KEY_MEETING", resp).catch(() => {});

  if (!resp.ok) {
    const err = await resp.text();
    throw new Error(`NIM API error: ${resp.status} ${err}`);
  }
...
function parseInsightResponse(text: string, meetingId: string): Record<string, unknown> {
  try {
    ...
    return {
      ...
      recurringIssues: Array.isArray(parsed.recurringIssues) ? parsed.recurringIssues : [],
      undecidedMatters: Array.isArray(parsed.undecidedMatters) ? parsed.undecidedMatters : [],
      bottlenecks: Array.isArray(parsed.bottlenecks) ? parsed.bottlenecks : [],
      aiSummary: parsed.aiSummary || "분석 결과를 생성하지 못했습니다.",
    };
  } catch {
