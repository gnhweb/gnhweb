// Central server-side AI router.
// Keys remain server-side. Provider-specific models, time budgets, fallback,
// cooldowns, response caching, truncation checks, and latency-aware ordering are centralized here.
...
function markCooldown(name: string, durationMs = COOLDOWN_MS) { cooldownUntil.set(name, Date.now() + durationMs); }
...
function resolveModel(provider: Provider) { return env(provider.modelEnv) || provider.defaultModel; }
...
async function requestCohere(provider: Provider, options: ChatOptions, model: string, signal: AbortSignal) {
  ...
  return fetch('https://api.cohere.com/v2/chat', {
    method: 'POST', signal, headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${providerKey(provider)}` },
    body: JSON.stringify({ model, ...(system ? { preamble: system } : {}), messages: [...history, { role: 'USER', message: last }], temperature: options.temperature ?? 0.4, max_tokens: outputBudget(options.task, options.maxTokens) }),
  });
}
...
const PROVIDERS: Provider[] = [
  ...
  makeProvider('huggingface', 'HUGGINGFACE_API_KEY', 'HUGGINGFACE_MODEL', 'openai/gpt-oss-120b:fastest', 'https://router.huggingface.co/v1/chat/completions'),
...
function taskOrder(task: string) {
  const t = task.toLowerCase();
  if (/(coaching|leadership|pastoral|리더|상담|사명)/i.test(t)) return ['gemini', 'mistral', 'groq', 'cerebras', 'cohere', 'sambanova', 'together', 'huggingface', 'openrouter', 'cloudflare', 'nvidia'];
  if (/(event|idea|creative|pds|행사|아이디어|기획)/i.test(t)) return ['gemini', 'groq', 'mistral', 'cerebras', 'huggingface', 'cohere', 'sambanova', 'together', 'openrouter', 'cloudflare', 'nvidia'];
  return ['gemini', 'groq', 'cerebras', 'mistral', 'huggingface', 'cohere', 'sambanova', 'together', 'openrouter', 'cloudflare', 'nvidia'];
}
function orderedProviders(task: string, configured: Provider[]) {
  ...
  return [...configured].sort((a, b) => {
    const ar = rank.get(a.name) ?? 999, br = rank.get(b.name) ?? 999;
    if (ar !== br) return ar - br;
    return (Number.isFinite(medianLatency(a.name)) ? medianLatency(a.name) : 9999) - (Number.isFinite(medianLatency(b.name)) ? medianLatency(b.name) : 9999);
  });
}
...
function pruneCache() { const now = Date.now(); for (const [key, value] of responseCache) if (value.expiresAt <= now) responseCache.delete(key); }
function looksTruncated(content: string, task: string) {
  const expectsJson = /(event|pds|action|mbti|quiz|letter|행사|기획|아이디어|퀴즈|편지)/i.test(task);
  if (!expectsJson) return false;
  const trimmed = content.trim();
  if (!trimmed.startsWith('{') && !trimmed.startsWith('```json')) return false;
  try { JSON.parse(trimmed.replace(/^```json\s*/i, '').replace(/```\s*$/i, '').trim()); return false; }
  catch { return true; }
}
...
export async function callAI(options: ChatOptions): Promise<Response> {
  ...
  for (const provider of orderedProviders(options.task, configured)) {
    if (attempts >= MAX_ATTEMPTS || TOTAL_BUDGET_MS - (Date.now() - start) < 500) break;
