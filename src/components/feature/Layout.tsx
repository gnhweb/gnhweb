...
export default function Layout(){
 ...
 const special=location.pathname==='/faith'?<FaithHubPage/>:location.pathname==='/telegram-settings'?<TelegramSettingsPage/>:location.pathname==='/bible-pick'?<BiblePick/>:location.pathname==='/bible-mbti'?<BibleMbtiEnhanced/>:location.pathname==='/leadership-diary'?<AuthGuard minRole="assistant_zone_leader"><Suspense fallback={<div className="min-h-[40vh] flex items-center justify-center p-6 text-sm text-muted-foreground">로딩 중…</div>}><LeadershipDiary/></Suspense></AuthGuard>:null;
