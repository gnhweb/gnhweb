...
function withTimeout<T>(promise: Promise<T>, ms: number): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timer = window.setTimeout(() => reject(new Error('TIMEOUT')));
    promise.then(
...
export default function AppLockScreen() {
  ...
  const handleDigit = (d: string) => {
    ...
    const nextPin = `${pin}${d}`;
    setPin(nextPin);
    if (nextPin.length === pinLength) void handleSubmit(nextPin);
  };
  ...
  return (
    <div
      className="fixed inset-0 z-[100] min-h-dvh bg-background-50 flex items-center justify-center px-4 py-[max(1rem,env(safe-area-inset-top))]"
      style={{ position: 'fixed', inset: 0, width: '100vw', height: '100dvh', minHeight: '100dvh', zIndex: 10000, overflowY: 'auto' }}
    >
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-xs text-center">
        ...
        <motion.div animate={shake ? { x: [0, -8, 8, -8, 8, 0] } : {}} transition={{ duration: 0.4 }} className="flex items-center justify-center gap-2.5 mb-4">
          {Array.from({ length: pinLength }).map((_, i) => <div key={i} className={`w-3.5 h-3.5 rounded-full border-2 transition-colors ${i < pin.length ? 'bg-amber-500 border-amber-500' : 'border-background-300'}`} />)}
        </motion.div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {['1','2','3','4','5','6','7','8','9'].map(d => <button key={d} type="button" onPointerUp={(e) => { e.preventDefault(); handleDigit(d); }} disabled={checking || passkeyChecking} className="h-14 rounded-2xl bg-background-100 hover:bg-background-200 text-lg font-semibold text-foreground-800 transition-colors cursor-pointer disabled:opacity-50">{d}</button>)}
          <div />
          ...
          <button type="button" onPointerUp={(e) => { e.preventDefault(); handleBackspace(); }} disabled={checking || passkeyChecking} className="h-14 rounded-2xl flex items-center justify-center text-foreground-500 hover:text-foreground-800 transition-colors cursor-pointer disabled:opacity-50" aria-label="PIN 지우기"><i className="ri-arrow-left-line text-xl" /></button>
        </div>
        {hasRegisteredBiometric && !checking && !passkeyChecking && <button type="button" onClick={() => void handleBiometricUnlock()} className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full border-2 border-amber-500 bg-amber-900/90 text-amber-50 shadow-sm transition-all hover:scale-105 active:scale-95 cursor-pointer" aria-label="등록된 지문 / Face ID로 잠금 해제"><i className="ri-fingerprint-line text-2xl" /></button>}
        {!confirmingLogout ? <button type="button" onClick={() => setConfirmingLogout(true)} className="text-sm text-foreground-500 hover:text-amber-600 transition-colors cursor-pointer">비밀번호를 잊으셨나요?</button> : <div className="bg-background-100 border border-background-200 rounded-xl p-3"><p className="text-xs text-foreground-600 mb-2">이메일과 비밀번호로 다시 로그인해주세요.</p><div className="flex gap-2 justify-center"><button type="button" onClick={() => setConfirmingLogout(false)} className="px-3 py-1.5 rounded-full text-xs text-foreground-600 hover:bg-background-200 cursor-pointer whitespace-nowrap">취소</button><button type="button" onClick={() => void signOut()} className="px-3 py-1.5 rounded-full bg-amber-500 text-white text-xs font-semibold hover:bg-amber-600 cursor-pointer">이메일로 로그인하기</button></div></div>}
      </motion.div>
    </div>
  );
}
