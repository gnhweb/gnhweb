...
export default function AppLockScreen() {
  ...
  const [pinLength] = useState(() => (
    user ? getSimplePinLength(user.id) : 4
  ));
  ...
  // Touch devices: do not autofocus on entry, so iOS/Android does not jump or open
  // the software keyboard unexpectedly. Physical/desktop keyboard users keep focus.
  useEffect(() => {
    const isTouchDevice =
      ...
      >
        <div className="w-16 h-16 rounded-full bg-amber-100 border border-amber-200 flex items-center justify-center mx-auto mb-5">
          {profile?.profile_image ? (
            ...
            <i className="ri-lock-2-line text-2xl text-amber-600" />
          )}
        </div>
