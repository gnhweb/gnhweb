...
export default function Navbar() {
  ...
  return <><nav className={`sticky top-0 z-40 pt-safe transition-all duration-300 ${scrolled ? 'bg-background-100/95 backdrop-blur-md shadow-sm border-b border-background-200/60 max-md:rounded-b-2xl max-md:shadow-card max-md:bg-background-100/75 max-md:backdrop-blur-lg max-md:border-b-0' : 'bg-transparent'}`}><div className="max-w-6xl mx-auto px-4 md:px-6"><div className={`relative flex justify-center items-center py-2 md:py-3 transition-all duration-300 ${scrolled ? 'max-md:py-1.5' : ''}`}><Link to="/" className="flex items-center gap-2 group cursor-pointer max-w-[calc(100%-64px)] md:max-w-none"><div className="w-8 h-8 rounded-lg bg-primary-100 max-md:bg-gradient-to-br max-md:from-primary-400 max-md:to-accent-400 flex items-center justify-center group-hover:bg-primary-200 transition-colors flex-shrink-0"><i className="ri-cross-line text-primary-600 max-md:text-white text-lg"></i></div><span className="font-bold text-foreground-950 text-sm md:text-base leading-tight truncate max-md:whitespace-normal max-md:line-clamp-2 md:whitespace-nowrap">스스로 신앙하는 거침없는 강릉 학생회</span></Link><ThemeToggleButton className="absolute right-0 top-1/2 -translate-y-1/2" /></div><div className="border-t border-primary-100/60"></div><div className="relative flex items-center justify-between h-11 md:h-12"><div className="hidden md:flex items-center gap-1">{TOP_ITEMS.map(item => <Link key={item.path} to={item.path} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 whitespace-nowrap cursor-pointer hover:scale-[1.02] ${isActive(item.path) ? 'bg-primary-100 text-primary-700' : 'text-foreground-600 hover:text-foreground-950 hover:bg-background-100'}`}><i className={`${item.icon} text-xs`}></i>{item.label}</Link>)}<form onSubmit={(e) => { e.preventDefault(); const q = globalSearch.trim(); if (q) navigate(`/search?q=${encodeURIComponent(q)}`); else navigate('/search'); }} className="relative ml-1 hidden xl:block"><i className="ri-search-line pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-foreground-400"></i><input value={globalSearch} onChange={(e) => setGlobalSearch(e.target.value)} placeholder="검색" aria-label="검색" className="w-32 rounded-full border border-background-200 bg-background-50 py-1.5 pl-8 pr-3 text-sm outline-none transition focus:w-44 focus:border-primary-300 focus:ring-4 focus:ring-primary-50" /></form><div className="w-px h-5 bg-background-200 mx-1"></div><div className="relative" ref={bibleRef}><button onClick={() => { setBibleOpen(!bibleOpen); setFaithOpen(false); setCommOpen(false); setMissionOpen(false); setAdminOpen(false); setProfileOpen(false); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 whitespace-nowrap cursor-pointer hover:scale-[1.02] ${bibleOpen ? catBgActive('amber') : `text-foreground-600 ${catBgHover('amber')}`}`}><i className="ri-book-open-line text-xs"></i>말씀 도구<i className={`ri-arrow-down-s-line text-xs transition-transform duration-200 ${bibleOpen ? 'rotate-180' : ''}`}></i></button><AnimatePresence>{bibleOpen && <motion.div initial={{ opacity: 0, y: -4, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.96 }} transition={{ duration: 0.15 }} className="absolute left-0 top-full mt-2 w-56 bg-background-100 rounded-xl shadow-lg border border-background-200 overflow-hidden"><div className="p-1.5 max-h-[380px] overflow-y-auto">{BIBLE_CATEGORY.items.map(item => <Link key={item.path} to={item.path} onClick={() => setBibleOpen(false)} className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${isActive(item.path) ? 'bg-amber-50 text-amber-700' : 'text-foreground-700 hover:bg-amber-50/70'}`}><i className={`${item.icon} text-amber-400`}></i>{item.label}</Link>)}</div></motion.div>}</AnimatePresence></div><div className="relative" ref={commRef}><button onClick={() => { setCommOpen(!commOpen); setBibleOpen(false); setFaithOpen(false); setMissionOpen(false); setAdminOpen(false); setProfileOpen(false); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 whitespace-nowrap cursor-pointer hover:scale-[1.02] ${commOpen ? catBgActive('emerald') : `text-foreground-600 ${catBgHover('emerald')}`} `}><i className="ri-group-line text-xs"></i>소통·공동체<i className={`ri-arrow-down-s-line text-xs transition-transform duration-200 ${commOpen ? 'rotate-180' : ''}`}></i></button><AnimatePresence>{commOpen && <motion.div initial={{ opacity: 0, y: -4, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.96 }} transition={{ duration: 0.15 }} className="absolute left-0 top-full mt-2 w-52 bg-background-100 rounded-xl shadow-lg border border-background-200 overflow-hidden"><div className="p-1.5 max-h-[380px] overflow-y-auto">{COMMUNITY_CATEGORY.items.map(item => <Link key={item.path} to={item.path} onClick={() => setCommOpen(false)} className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${isActive(item.path) ? 'bg-emerald-50 text-emerald-700' : 'text-foreground-700 hover:bg-emerald-50/70'}`}><i className={`${item.icon} text-emerald-400`}></i>{item.label}</Link>)}</div></motion.div>}</AnimatePresence></div><div className="relative" ref={gameRef}><button onClick={() => { setGameOpen(!gameOpen); setBibleOpen(false); setFaithOpen(false); setCommOpen(false); setMissionOpen(false); setAdminOpen(false); setProfileOpen(false); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 whitespace-nowrap cursor-pointer hover:scale-[1.02] ${gameOpen ? catBgActive('indigo') : `text-foreground-600 ${catBgHover('indigo')}`}`}><i className="ri-gamepad-line text-xs"></i>갓겜<i className={`ri-arrow-down-s-line text-xs transition-transform duration-200 ${gameOpen ? 'rotate-180' : ''}`}></i></button><AnimatePresence>{gameOpen && <motion.div initial={{ opacity: 0, y: -4, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.96 }} transition={{ duration: 0.15 }} className="absolute left-0 top-full mt-2 w-52 bg-background-100 rounded-xl shadow-lg border border-background-200 overflow-hidden"><div className="p-1.5 max-h-[380px] overflow-y-auto">{GAME_CATEGORY.items.map(item => <Link key={item.path} to={item.path} onClick={() => setGameOpen(false)} className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${isActive(item.path) ? 'bg-indigo-50 text-indigo-700' : 'text-foreground-700 hover:bg-indigo-50/70'}`}><i className={`${item.icon} text-indigo-400`}></i>{item.label}</Link>)}</div></motion.div>}</AnimatePresence></div><div className="w-px h-5 bg-background-200 mx-1"></div><div className="relative" ref={faithRef}><button onClick={() => { setFaithOpen(!faithOpen); setBibleOpen(false); setCommOpen(false); setGameOpen(false); setMissionOpen(false); setAdminOpen(false); setProfileOpen(false); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 whitespace-nowrap cursor-pointer hover:scale-[1.02] ${faithOpen ? catBgActive('primary') : `text-foreground-600 ${catBgHover('primary')}`} `}><i className="ri-lock-line text-xs"></i>신앙(비공개)<i className={`ri-arrow-down-s-line text-xs transition-transform duration-200 ${faithOpen ? 'rotate-180' : ''}`}></i></button><AnimatePresence>{faithOpen && <motion.div initial={{ opacity: 0, y: -4, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.96 }} transition={{ duration: 0.15 }} className="absolute left-0 top-full mt-2 w-52 bg-background-100 rounded-xl shadow-lg border border-background-200 overflow-hidden"><div className="p-1.5 max-h-[380px] overflow-y-auto">{FAITH_CATEGORY.items.map(item => <Link key={item.path} to={item.path} onClick={() => setFaithOpen(false)} className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${isActive(item.path) ? 'bg-primary-50 text-primary-700' : 'text-foreground-700 hover:bg-primary-50/70'}`}><i className={`${item.icon} text-primary-400`}></i>{item.label}</Link>)}</div></motion.div>}</AnimatePresence></div>{showMissionTab && <div className="relative" ref={missionRef}><button onClick={() => { setMissionOpen(!missionOpen); setBibleOpen(false); setFaithOpen(false); setCommOpen(false); setAdminOpen(false); setProfileOpen(false); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold transition-all duration-200 whitespace-nowrap cursor-pointer hover:scale-[1.02] ${missionOpen ? catBgActive('rose') : `text-accent-600 ${catBgHover('rose')}`} `}><i className="ri-shield-star-line text-xs"></i>사명자<i className={`ri-arrow-down-s-line text-xs transition-transform duration-200 ${missionOpen ? 'rotate-180' : ''}`}></i></button><AnimatePresence>{missionOpen && <motion.div initial={{ opacity: 0, y: -4, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.96 }} transition={{ duration: 0.15 }} className="absolute left-0 top-full mt-2 w-60 bg-background-100 rounded-xl shadow-lg border border-background-200 overflow-hidden"><div className="p-1.5 max-h-[440px] overflow-y-auto">{showTeacherTab && <><Link to="/teacher-dashboard" onClick={() => setMissionOpen(false)} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer text-accent-700 hover:bg-accent-50"><i className="ri-dashboard-line text-accent-400"></i>교사 대시보드</Link><div className="border-t border-background-200 my-1"></div></>}{MISSION_SUBSECTIONS.map(section => <div key={section.label} className="mb-1"><p className="px-3 py-1.5 text-[11px] font-semibold text-foreground-400 uppercase tracking-wider">{section.label}</p>{section.items.map(item => <button key={item.label} onClick={() => handleMissionAction(item)} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm text-foreground-700 hover:bg-accent-50 hover:text-accent-700 transition-colors cursor-pointer text-left"><i className={`${item.icon} text-accent-300`}></i>{item.label}</button>)}</div>)}</div></motion.div>}</AnimatePresence></div>}{(showAdminTab || showTeacherTab) && visibleAdminItems.length > 0 && <div className="relative" ref={adminRef}><button onClick={() => { setAdminOpen(!adminOpen); setBibleOpen(false); setFaithOpen(false); setCommOpen(false); setMissionOpen(false); setProfileOpen(false); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold transition-all duration-200 whitespace-nowrap cursor-pointer hover:scale-[1.02] ${adminOpen ? catBgActive('slate') : `text-secondary-600 ${catBgHover('slate')}`} `}><i className="ri-settings-3-line text-xs"></i>관리<i className={`ri-arrow-down-s-line text-xs transition-transform duration-200 ${adminOpen ? 'rotate-180' : ''}`}></i></button><AnimatePresence>{adminOpen && <motion.div initial={{ opacity: 0, y: -4, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.96 }} transition={{ duration: 0.15 }} className="absolute left-0 top-full mt-2 w-52 bg-background-100 rounded-xl shadow-lg border border-background-200 overflow-hidden"><div className="p-1.5">{visibleAdminItems.map(item => <Link key={item.label} to={item.path} onClick={() => setAdminOpen(false)} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm text-foreground-700 hover:bg-secondary-50 hover:text-secondary-700 transition-colors cursor-pointer"><i className={`${item.icon} text-secondary-400`}></i>{item.label}</Link>)}</div></motion.div>}</AnimatePresence></div>}</div><div className="hidden md:flex items-center gap-3">{user && <button onClick={() => setNotificationsOpen(!notificationsOpen)} className="flex items-center justify-center w-9 h-9 rounded-full hover:bg-background-100 transition-colors cursor-pointer relative"><i className="ri-notification-3-line text-lg text-foreground-600"></i>{notificationCount > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-amber-500 text-white text-[9px] font-bold flex items-center justify-center">{notificationCount > 99 ? '99+' : notificationCount}</span>}</button>}{user ? <div className="relative" ref={profileRef}><button onClick={() => { setBibleOpen(false); setFaithOpen(false); setCommOpen(false); setMissionOpen(false); setAdminOpen(false); setProfileOpen(!profileOpen); }} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary-50 hover:bg-primary-100 transition-all duration-200 cursor-pointer hover:scale-[1.02]">{profile ? <><div className="w-7 h-7 rounded-full bg-amber-200 flex items-center justify-center"><span className="text-xs font-bold text-amber-700">{profile.name.charAt(0)}</span></div><span className="text-sm font-medium text-foreground-800 max-w-[80px] truncate">{profile.name}</span></> : <><div className="w-7 h-7 rounded-full bg-amber-100 flex items-center justify-center"><span className="w-3.5 h-3.5 border-2 border-amber-400 border-t-transparent rounded-full animate-spin"></span></div><span className="text-sm text-foreground-400">불러오는 중...</span></>}<i className={`ri-arrow-down-s-line text-foreground-400 text-xs transition-transform duration-200 ${profileOpen ? 'rotate-180' : ''}`}></i></button><AnimatePresence>{profileOpen && <motion.div initial={{ opacity: 0, y: -4, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.96 }} transition={{ duration: 0.15 }} className="absolute right-0 top-full mt-2 w-60 bg-background-100 rounded-xl shadow-lg border border-background-200 overflow-hidden z-50"><ProfileDropdownTabs faithItems={PROFILE_FAITH_ITEMS} activityItems={PROFILE_ACTIVITY_ITEMS} onClose={() => setProfileOpen(false)} onSignOut={handleSignOut} onSuggestions={handleSuggestions} /></motion.div>}</AnimatePresence></div> : <Link to="/login" className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary-50 hover:bg-primary-100 transition-all duration-200 cursor-pointer"><i className="ri-login-box-line text-primary-600"></i><span className="text-sm font-medium text-primary-700">로그인</span></Link>}</div></div></div><div className="flex md:hidden items-center justify-between h-11 w-full"><button onClick={handleMobileBack} aria-label="뒤로가기" className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-background-100 transition-colors cursor-pointer"><i className="ri-arrow-left-line text-lg text-foreground-600"></i></button><button onClick={() => setMobileOpen(!mobileOpen)} aria-label={mobileOpen ? '메뉴 닫기' : '메뉴 열기'} className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-background-100 transition-colors cursor-pointer"><i className={`text-lg ${mobileOpen ? 'ri-close-line' : 'ri-menu-line'}`}></i></button></div></div>{mobileOpen && <div className="fixed inset-0 z-50 md:hidden bg-background-100/95 backdrop-blur-sm" onClick={() => setMobileOpen(false)}><div className="absolute top-[calc(env(safe-area-inset-top)+5.5rem)] left-3 right-3 max-h-[calc(100vh-env(safe-area-inset-top)-6.5rem)] overflow-y-auto bg-background-100 rounded-3xl shadow-card-lg border border-background-200 p-3" onClick={e => e.stopPropagation()}><div className="px-2 pb-2"><div className="text-xs text-foreground-400 mb-1">빠른 메뉴</div><div className="grid grid-cols-3 gap-1">{TOP_ITEMS.map(item => <MenuGridCard key={`m-top-${item.path}`} icon={item.icon} label={item.label} colorClass="bg-primary-100 text-primary-600" active={isActive(item.path)} onClick={() => { navigate(item.path); setMobileOpen(false); }} />)}</div></div><div className="border-t border-background-200 my-2"></div><AccordionBlock icon="ri-book-open-line" label="말씀 도구" color="amber" open={!!mobileAccordion['bible']} onToggle={() => toggleMobileAccordion('bible')}><div className="grid grid-cols-3 gap-1">{BIBLE_CATEGORY.items.map(item => <MenuGridCard key={`m-bible-${item.path}`} icon={item.icon} label={item.label} colorClass="bg-amber-100 text-amber-600" active={isActive(item.path)} onClick={() => { navigate(item.path); setMobileOpen(false); }} />)}</div></AccordionBlock><AccordionBlock icon="ri-group-line" label="소통·공동체" color="emerald" open={!!mobileAccordion['community']} onToggle={() => toggleMobileAccordion('community')}><div className="grid grid-cols-3 gap-1">{COMMUNITY_CATEGORY.items.map(item => <MenuGridCard key={`m-community-${item.path}`} icon={item.icon} label={item.label} colorClass="bg-emerald-100 text-emerald-600" active={isActive(item.path)} onClick={() => { navigate(item.path); setMobileOpen(false); }} />)}</div></AccordionBlock><AccordionBlock icon="ri-gamepad-line" label="갓겜" color="indigo" open={!!mobileAccordion['game']} onToggle={() => toggleMobileAccordion('game')}><div className="grid grid-cols-3 gap-1">{GAME_CATEGORY.items.map(item => <MenuGridCard key={`m-game-${item.path}`} icon={item.icon} label={item.label} colorClass="bg-indigo-100 text-indigo-600" active={isActive(item.path)} onClick={() => { navigate(item.path); setMobileOpen(false); }} />)}</div></AccordionBlock><div className="flex items-center gap-2 px-2 py-2 mt-1 mb-2"><div className="h-px flex-1 bg-background-200"></div><span className="text-[10px] font-bold text-foreground-400 uppercase tracking-widest">나의 기록</span><div className="h-px flex-1 bg-background-200"></div></div><AccordionBlock icon="ri-lock-line" label="신앙(비공개)" color="primary" open={!!mobileAccordion['faith']} onToggle={() => toggleMobileAccordion('faith')}><div className="grid grid-cols-3 gap-1">{FAITH_CATEGORY.items.map(item => <MenuGridCard key={`m-faith-${item.path}`} icon={item.icon} label={item.label} colorClass="bg-primary-100 text-primary-600" active={isActive(item.path)} onClick={() => { navigate(item.path); setMobileOpen(false); }} />)}</div></AccordionBlock>{showMissionTab && <AccordionBlock icon="ri-shield-star-line" label="사명자 전용" color="rose" open={!!mobileAccordion['mission']} onToggle={() => toggleMobileAccordion('mission')}><div className="space-y-3">{showTeacherTab && <div className="grid grid-cols-2 gap-1"><MenuGridCard icon="ri-dashboard-line" label="교사 대시보드" colorClass="bg-accent-100 text-accent-600" active={isActive('/teacher-dashboard')} onClick={() => { navigate('/teacher-dashboard'); setMobileOpen(false); }} /></div>}{MISSION_SUBSECTIONS.map(section => <div key={`m-ms-${section.label}`}><p className="px-2 py-1 text-[11px] font-semibold text-foreground-400 uppercase tracking-wider">{section.label}</p><div className="grid grid-cols-3 gap-1">{section.items.map(item => <MenuGridCard key={`m-msi-${item.label}`} icon={item.icon} label={item.label} colorClass="bg-accent-100 text-accent-600" active={!!item.path && isActive(item.path)} onClick={() => handleMissionAction(item)} />)}</div></div>)}</div></AccordionBlock>}{(showAdminTab || showTeacherTab) && visibleAdminItems.length > 0 && <AccordionBlock icon="ri-settings-3-line" label="관리" color="slate" open={!!mobileAccordion['admin']} onToggle={() => toggleMobileAccordion('admin')}><div className="grid grid-cols-3 gap-1">{visibleAdminItems.map(item => <MenuGridCard key={`m-admin-${item.label}`} icon={item.icon} label={item.label} colorClass="bg-secondary-100 text-secondary-600" active={isActive(item.path)} onClick={() => { navigate(item.path); setMobileOpen(false); }} />)}</div></AccordionBlock>}{user && <div className="mt-2 pt-3 border-t border-background-200 pb-8 space-y-1">{profile?.club && <Link to={`/clubs/${profile.club}/community`} onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-primary-600 hover:bg-primary-50 transition-colors cursor-pointer"><i className="ri-chat-smile-2-line"></i>내 동아리 소통방</Link>}<Link to="/dashboard/attendance" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-foreground-700 hover:bg-background-100 transition-colors cursor-pointer"><i className="ri-user-heart-line"></i>스마트 출석</Link><Link to="/missions/board" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-foreground-700 hover:bg-background-100 transition-colors cursor-pointer"><i className="ri-medal-line"></i>작은 사명</Link><Link to="/profile" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-primary-600 hover:bg-primary-50 transition-colors cursor-pointer font-medium"><i className="ri-user-settings-line"></i>프로필 설정</Link><button onClick={handleSignOut} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-foreground-700 hover:bg-background-100 transition-colors cursor-pointer"><i className="ri-logout-box-line"></i>로그아웃</button></div>}</div></div>}
      </div></nav>
