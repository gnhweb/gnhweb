...
const TABS: TabDef[] = [
  ...
  { key: 'game', label: '갓겜', icon: 'ri-gamepad-line', activeIcon: 'ri-gamepad-fill', path: '/games' },
  { key: 'quiz', label: '성경퀴즈', icon: 'ri-question-answer-line', activeIcon: 'ri-question-answer-fill', path: '/bible-quiz' },
  { key: 'more', label: '더보기', icon: 'ri-menu-line', activeIcon: 'ri-menu-fill' },
];
...
export default function BottomTabBar() {
  ...
  const isTabActive = (tab: TabDef) => {
    ...
    if (tab.key === 'game') return GAME_PLAY_PATHS.some(p => location.pathname.startsWith(p)) || location.pathname.startsWith(tab.path);
            ...
            const content = (
              ...
              >
                {isCenter ? (
                  ...
                  >
                    <i className={`${active ? tab.activeIcon : tab.icon} text-white text-2xl`}></i>
                  </div>
                ) : tab.key === 'more' && profile?.profile_image ? (
                  <div
                    className={`w-7 h-7 rounded-full overflow-hidden ${
                      active ? 'ring-2 ring-primary-500' : 'ring-1 ring-background-200'
                    }`}
                  >
                ...
                <span
                  className={`text-[10px] leading-none ${
                    active ? 'text-primary-600 font-bold' : 'text-foreground-400 font-medium'
                  } ${isCenter ? 'mt-0.5' : ''}`}
                >
                  {tab.label}
                </span>
