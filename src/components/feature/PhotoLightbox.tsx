...
export default function PhotoLightbox({ photos, thumbUrls, initialIndex, onClose, captions, onDelete, canDelete, deletingIndex }: PhotoLightboxProps) {
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (pointersRef.current.size === 1) {
      if (scale > 1 && panStartRef.current) {
        ...
        if (Math.abs(dy) > Math.abs(dx)) setDragOffset(dy);
      }
    }
  };
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    if (pointersRef.current.size === 0) {
      ...
      pinchStartRef.current = null;
      if (scale > 1) panStartRef.current = { x: remaining.x, y: remaining.y, tx: translate.x, ty: translate.y };
    }
  };
    ...
    <AnimatePresence>
      ...
      >
        <div className="relative z-10 flex items-center justify-between px-4 pt-3 pb-2 shrink-0" onClick={e => e.stopPropagation()}>
          <span className="text-white/80 text-xs font-medium tabular-nums">{photos.length > 1 ? `${index + 1} / ${photos.length}` : ''}</span>
          <div className="flex items-center gap-2">
            {onDelete && (!canDelete || canDelete(index)) && (
              <button onClick={() => { if (window.confirm('이 사진을 삭제할까요? 이 작업은 되돌릴 수 없습니다.')) onDelete(index); }} disabled={deletingIndex === index} aria-label="삭제" className="w-9 h-9 rounded-full bg-white/10 hover:bg-rose-500/70 text-white flex items-center justify-center cursor-pointer active:scale-90 transition-transform disabled:opacity-50">
                <i className={`${deletingIndex === index ? 'ri-loader-4-line animate-spin' : 'ri-delete-bin-line'} text-lg`}></i>
              </button>
            )}
            <button onClick={onClose} aria-label="닫기" className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center cursor-pointer active:scale-90 transition-transform">
        ...
        <div ref={containerRef} className="relative flex-1 flex items-center justify-center overflow-hidden" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp} onClick={e => e.stopPropagation()}>
          ...
          {photos.length > 1 && (
            <>
              {index > 0 && <button onClick={(e) => { e.stopPropagation(); goTo(index - 1); }} aria-label="이전 사진" className="hidden md:flex absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white items-center justify-center cursor-pointer"><i className="ri-arrow-left-s-line text-2xl"></i></button>}
              {index < photos.length - 1 && <button onClick={(e) => { e.stopPropagation(); goTo(index + 1); }} aria-label="다음 사진" className="hidden md:flex absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white items-center justify-center cursor-pointer"><i className="ri-arrow-right-s-line text-2xl"></i></button>}
            </>
          )}
        </div>

        <div className="relative z-10 shrink-0" onClick={e => e.stopPropagation()}>
          {currentCaption && <p className="text-center text-white/85 text-sm px-6 pb-2 truncate">{currentCaption}</p>}
          {photos.length > 1 && (
            <div className="flex gap-2 overflow-x-auto px-4 pb-4 pt-1 no-scrollbar">
              {photos.map((p, i) => (
                <button key={i} onClick={() => goTo(i)} className={`shrink-0 w-12 h-12 rounded-lg overflow-hidden border-2 transition-colors cursor-pointer ${i === index ? 'border-white' : 'border-transparent opacity-50'}`}>
