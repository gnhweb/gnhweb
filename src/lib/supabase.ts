...
const MIGRATED_CLOUDFLARE_FUNCTIONS = new Set(['meeting-ideas-ai', 'meeting-insight-ai', 'nim-letter', 'nim-coaching', 'nim-counseling', 'nim-quiz', 'nim-mbti', 'prayer-relay']);
const cloudflareFunctions = new Proxy(legacySupabase.functions, { get(target, property, receiver) { if (property !== 'invoke') return Reflect.get(target, property, receiver); return async (functionName: string, options?: { body?: unknown }) => { if (!MIGRATED_CLOUDFLARE_FUNCTIONS.has(functionName)) return target.invoke(functionName, options); const endpoint = functionName === 'meeting-ideas-ai' ? '/meeting-ideas' : functionName === 'meeting-insight-ai' ? '/meeting-insight' : functionName === 'nim-letter' ? '/nim-letter' : functionName === 'nim-coaching' ? '/nim-coaching' : functionName === 'nim-counseling' ? '/nim-counseling' : functionName === 'nim-quiz' ? '/nim-quiz' : functionName === 'nim-mbti' ? '/nim-mbti' : '/prayer-relay'; try { const jwt = functionName === 'prayer-relay' ? await neonAuth.getJWTToken?.() : null; const headers: Record<string, string> = { 'Content-Type': 'application/json' }; if (jwt) headers.Authorization = `Bearer ${jwt}`; const response = await fetch(`${functionName === 'prayer-relay' ? CLOUDFLARE_API : CLOUDFLARE_AI_GATEWAY}${endpoint}`, { method: 'POST', headers, body: JSON.stringify(options?.body ?? {}) }); const text = await response.text(); let data: unknown = null; try { data = text ? JSON.parse(text) : null; } catch { data = null; } if (!response.ok) return { data: null, error: new Error(typeof (data as { error?: unknown })?.error === 'string' ? (data as { error: string }).error : `Cloudflare request failed: ${response.status}`) }; return { data, error: null }; } catch (error) { return { data: null, error: error instanceof Error ? error : new Error(String(error)) }; } }; } });
