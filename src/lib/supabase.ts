...
const MIGRATED_CLOUDFLARE_FUNCTIONS = new Set(['meeting-ideas-ai', 'meeting-insight-ai', 'nim-letter', 'nim-coaching', 'nim-counseling', 'nim-quiz', 'nim-mbti', 'prayer-relay', 'quiz-leaderboard', 'quiz-report', 'streak-tracker', 'bible-streak-update', 'bible-pick', 'monthly-champion-snapshot']);
const cloudflareFunctions = new Proxy(legacySupabase.functions, { get(target, property, receiver) { if (property !== 'invoke') return Reflect.get(target, property, receiver); return async (functionName: string, options?: { body?: unknown; method?: string }) => { const separatorIndex = functionName.indexOf('?'); const baseFunctionName = separatorIndex >= 0 ? functionName.slice(0, separatorIndex) : functionName; const query = separatorIndex >= 0 ? functionName.slice(separatorIndex) : ''; if (!MIGRATED_CLOUDFLARE_FUNCTIONS.has(baseFunctionName)) return target.invoke(functionName, options); const endpoint = baseFunctionName === 'meeting-ideas-ai' ? '/meeting-ideas' : baseFunctionName === 'meeting-insight-ai' ? '/meeting-insight' : baseFunctionName === 'nim-letter' ? '/nim-letter' : baseFunctionName === 'nim-coaching' ? '/nim-coaching' : baseFunctionName === 'nim-counseling' ? '/nim-counseling' : baseFunctionName === 'nim-quiz' ? '/nim-quiz' : baseFunctionName === 'nim-mbti' ? '/nim-mbti' : baseFunctionName === 'prayer-relay' ? '/prayer-relay' : baseFunctionName === 'quiz-leaderboard' ? '/quiz-leaderboard' : baseFunctionName === 'quiz-report' ? '/quiz-report' : baseFunctionName === 'streak-tracker' ? '/streak-tracker' : baseFunctionName === 'bible-streak-update' ? '/bible-streak-update' : baseFunctionName === 'bible-pick' ? '/bible-pick' : '/monthly-champion-snapshot'; try { const jwtRequiredFunction = baseFunctionName === 'prayer-relay' || baseFunctionName === 'quiz-leaderboard' || baseFunctionName === 'quiz-report' || baseFunctionName === 'streak-tracker' || baseFunctionName === 'bible-streak-update' || baseFunctionName === 'bible-pick' || baseFunctionName === 'monthly-champion-snapshot'; const jwt = jwtRequiredFunction ? await neonAuth.getJWTToken?.() : null; const headers: Record<string, string> = { 'Content-Type': 'application/json' }; if (jwt) headers.Authorization = `Bearer ${jwt}`; const method = options?.method || 'POST'; const response = await fetch(`${CLOUDFLARE_API}${endpoint}${query}`, { method, headers, body: method === 'GET' || method === 'HEAD' ? undefined : JSON.stringify(options?.body ?? {}) }); const text = await response.text(); let data: unknown = null; try { data = text ? JSON.parse(text) : null; } catch { data = null; } if (!response.ok) return { data: null, error: new Error(typeof (data as { error?: unknown })?.error === 'string' ? (data as { error: string }).error : `Cloudflare request failed: ${response.status}`) }; return { data, error: null }; } catch (error) { return { data: null, error: error instanceof Error ? error : new Error(String(error)) }; }; } });
