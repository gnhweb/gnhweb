...
const DIFFICULTIES = [
  { key: 'easy' as const, label: '입문', color: 'text-emerald-600', bg: 'bg-emerald-100', border: 'border-emerald-200', scorePerQ: 10, stars: 1, gradient: 'from-emerald-400 to-emerald-500', icon: 'ri-seedling-line' },
  { key: 'normal' as const, label: '보통', color: 'text-amber-600', bg: 'bg-amber-100', border: 'border-amber-200', scorePerQ: 20, stars: 2, gradient: 'from-amber-400 to-amber-500', icon: 'ri-flashlight-line' },
  { key: 'hard' as const, label: '도전', color: 'text-rose-600', bg: 'bg-rose-100', border: 'border-rose-200', scorePerQ: 30, stars: 3, gradient: 'from-rose-400 to-rose-500', icon: 'ri-fire-line' },
];
...
export default function BibleQuiz() {
  ...
  // 뒤로가기: 퀴즈 중 난이도 선택 화면으로 돌아감 (문제 초기화)
  ...
  const scorePerQ = diffInfo?.scorePerQ || 10;
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-10 md:py-16">
          ...
          {cumulativeStats && cumulativeStats.games_played > 0 && (
            <div className="mt-4 inline-flex items-center gap-3 px-5 py-2.5 bg-amber-50 border border-amber-200 rounded-full">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-bold text-amber-700">{cumulativeStats.total_score.toLocaleString()}점</span>
              </div>
              <div className="w-px h-4 bg-amber-200"></div>
              <div className="flex items-center gap-1.5">
                <i className="ri-gamepad-line text-amber-600 text-sm"></i>
                <span className="text-xs font-medium text-amber-700">{cumulativeStats.games_played}회</span>
              </div>
              <div className="w-px h-4 bg-amber-200"></div>
              <div className="flex items-center gap-1.5">
                <i className="ri-check-double-line text-amber-600 text-sm"></i>
                <span className="text-xs font-medium text-amber-700">정답률 {cumulativeStats.accuracy}%</span>
              </div>
            </div>
          )}
        </motion.div>
        ...
        {questions.length === 0 && (
          ...
          >
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-amber-50 border-2 border-amber-200 text-amber-700 font-semibold text-sm hover:bg-amber-100 transition-all cursor-pointer whitespace-nowrap"
            >
          ...
          >
            ...
            <div className="mb-6">
                ...
                {DIFFICULTIES.map(d => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`relative flex flex-col items-center gap-1.5 py-4 px-2 rounded-[20px] transition-all cursor-pointer overflow-hidden ${
                        active
                          ? `bg-gradient-to-br ${d.gradient} shadow-card-lg`
                          : 'bg-background-50 border border-background-200'
                      }`}
                    >
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center ${active ? 'bg-background-100/25' : d.bg}`}>
                        <i className={`${d.icon} text-lg ${active ? 'text-white' : d.color}`}></i>
                      </div>
                        ...
                        {Array.from({ length: 3 }).map((_, i) => (
                          <i
                            ...
                            className={`ri-star-fill text-[10px] ${
                              i < d.stars ? (active ? 'text-white' : 'text-amber-400') : (active ? 'text-white/30' : 'text-background-300')
                            }`}
                          ></i>
                        ))}
                      </div>
              ...
              <p className="text-xs text-foreground-400 text-center mt-3">
                {difficulty === 'easy' ? '입문: 문제당 10점 — 기본 성경 상식 문제' :
                 difficulty === 'hard' ? '도전: 문제당 30점 — 높은 점수와 함께 심화 문제' :
                 '보통: 문제당 20점 — 중급 성경 지식 문제'}
              </p>
            </div>
            ...
            <button
              onClick={() => startQuiz()}
              disabled={isLoading}
              className="w-full py-3.5 rounded-[20px] bg-secondary-500 text-background-50 font-semibold text-base hover:bg-secondary-600 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer whitespace-nowrap"
            >
            ...
            {error && (
              <div className="mt-4 p-3 rounded-xl bg-accent-100 border border-accent-200 text-sm text-accent-700 flex items-start gap-2">
        ...
        {isLoading && questions.length === 0 && (
          <div className="flex items-center justify-center py-8">
            <div className="w-6 h-6 rounded-full border-2 border-secondary-400 border-t-transparent animate-spin mr-3"></div>
            <span className="text-sm text-foreground-500">문제 준비 중...</span>
          </div>
        )}
          ...
          >
            ...
            <div className="flex items-center justify-between mb-2">
              <button
                ...
                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-background-200 text-xs font-medium text-foreground-600 hover:bg-background-300 transition-colors cursor-pointer"
              >
              ...
              <div className="flex items-center gap-2">
                {streak >= 2 && (
                  <span className="text-xs font-bold text-amber-600 flex items-center gap-1">
                    <i className="ri-fire-line"></i> x{streak}
                  </span>
                )}
                <div className={`text-xs font-bold px-2 py-1 rounded-full ${timer <= 5 ? 'bg-rose-100 text-rose-600' : 'bg-background-200 text-foreground-600'}`}>
                  {timer}s
                </div>
              </div>
            </div>

            {/* 진행률 바 — 큼직하게, 그라디언트 채워짐 */}
            <div className="mb-3">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-bold text-foreground-600">{currentQ + 1} / {questions.length}</span>
                <span className={`text-xs px-2.5 py-1 rounded-full font-bold ${
                  questions[currentQ]?.type === 'ox' ? 'bg-primary-100 text-primary-700' :
                  'bg-secondary-100 text-secondary-700'
                }`}>
                  {questions[currentQ]?.type === 'ox' ? 'O/X' : '객관식'}
                </span>
              </div>
              <div className="h-2.5 rounded-full bg-background-200 overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${((currentQ + (selectedAnswer !== null ? 1 : 0)) / questions.length) * 100}%` }}
            ...
            {currentQ === questions.length - 1 && (
              ...
              >
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-100 text-rose-700 text-xs font-bold">
                  <i className="ri-vip-crown-line"></i> 보스 라운드! 보너스 점수 2배!
                </span>
              </motion.div>
            )}
            ...
            <div className="bg-background-100 border border-background-200 rounded-[20px] p-6 md:p-8">
                ...
                {questions[currentQ]?.options.map((option, idx) => {
                  ...
                  return (
                    <motion.button
                      ...
                      transition={{ duration: isWrongPick ? 0.4 : 0.25, delay: isWrongPick ? 0 : idx * 0.06 }}
                      onClick={() => handleAnswer(option)}
                      disabled={selectedAnswer !== null}
                      className={`w-full min-h-[56px] text-left px-4 py-4 rounded-2xl transition-colors duration-150 cursor-pointer group ${cardStyle}`}
                    >
              ...
              {/* Explanation */}
              {isCorrect !== null && selectedAnswer !== 'TIMEOUT' && (
                <motion.div
                  initial={{ opacity: 0, height: 0 }}
                  animate={{ opacity: 1, height: 'auto' }}
              ...
              {selectedAnswer !== null && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-5 text-center">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-secondary-500 text-background-50 font-semibold text-sm hover:bg-secondary-600 transition-all duration-300 cursor-pointer whitespace-nowrap"
                  >
              ...
              <div className="mt-5 text-center">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-xs text-foreground-400 hover:text-rose-500 transition-colors cursor-pointer whitespace-nowrap"
                >
        ...
        <AnimatePresence>
          {showResult && (
            <motion.div
              ...
              className="bg-background-100 border border-background-200 rounded-[20px] p-6 md:p-8 text-center"
            >
              <div className="w-20 h-20 rounded-full bg-secondary-100 border-4 border-secondary-200 flex items-center justify-center mx-auto mb-5">
                <i className="ri-trophy-line text-4xl text-secondary-600"></i>
              </div>
              <h2 className="text-xl font-bold text-foreground-950 mb-2">퀴즈 완료!</h2>
              <p className="text-xs text-foreground-500 mb-4">
                {autoClubInfo && <span>{autoClubInfo.name} · </span>}
                난이도: {difficulty === 'easy' ? '입문' : difficulty === 'hard' ? '도전' : '보통'} · 문제당 {scorePerQ}점
              </p>

              <div className="flex items-center justify-center gap-4 mb-4">
                <div className="text-center">
                  <p className="text-3xl font-black text-secondary-600">{score}</p>
                  <p className="text-xs text-foreground-500">이번 점수</p>
                </div>
                <div className="w-px h-10 bg-background-300"></div>
                <div className="text-center">
                  <p className="text-3xl font-black text-foreground-800">{correctCount}/{questions.length}</p>
                  <p className="text-xs text-foreground-500">정답</p>
                </div>
                <div className="w-px h-10 bg-background-300"></div>
                <div className="text-center">
                  <p className="text-3xl font-black text-amber-500">{maxStreak}</p>
                  <p className="text-xs text-foreground-500">최대 연속</p>
                </div>
              </div>
              ...
              {cumulativeStats && cumulativeStats.games_played > 0 && (
                ...
                >
                  <div className="flex items-center justify-center gap-1 mb-2">
                    ...
                    <span className="text-xs font-bold text-amber-700">누적 기록</span>
                  </div>
                  <div className="flex items-center justify-center gap-4">
                    <div className="text-center">
                      <p className="text-xl font-black text-amber-700">{cumulativeStats.total_score.toLocaleString()}</p>
                      <p className="text-xs text-amber-600">총 점수</p>
                    </div>
                    <div className="w-px h-8 bg-amber-200"></div>
                    <div className="text-center">
                      <p className="text-xl font-black text-amber-700">{cumulativeStats.games_played}</p>
                      <p className="text-xs text-amber-600">게임 수</p>
                    </div>
                    <div className="w-px h-8 bg-amber-200"></div>
                    <div className="text-center">
                      <p className="text-xl font-black text-amber-700">{cumulativeStats.accuracy}%</p>
                      <p className="text-xs text-amber-600">정답률</p>
                    </div>
                  </div>
                  {savingScore && (
                    <p className="text-xs text-amber-500 mt-2">점수 저장 중...</p>
                  )}
                </motion.div>
              )}
              ...
              {(() => {
                ...
                return (
                  <div className="flex items-center justify-center gap-2 mb-6">
                    <i className={`${rank.icon} ${rank.color} text-xl`}></i>
                    <p className={`text-sm font-bold ${rank.color}`}>{rank.text}</p>
                  </div>
                );
              })()}
              ...
              <div className="flex items-center justify-center gap-3 mt-6 flex-wrap">
                <button
                  onClick={handleBackToDifficulty}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-secondary-500 text-background-50 font-semibold text-sm hover:bg-secondary-600 transition-all cursor-pointer whitespace-nowrap"
                >
                ...
                <button
                  onClick={() => setShowLeaderboard(true)}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-amber-50 border-2 border-amber-200 text-amber-700 font-semibold text-sm hover:bg-amber-100 transition-all cursor-pointer whitespace-nowrap"
                >
        ...
        <ReportQuestionModal
          ...
          question={questions[currentQ] || null}
        />
      </div>
    </div>
  );
    ...
    >
      {Array.from({ length: 40 }).map((_, i) => (
        <motion.div
          ...
          style={{
            backgroundColor: ['#f59e0b', '#10b981', '#0ea5e9', '#f43f5e', '#8b5cf6', '#ec4899'][i % 6],
            left: '50%',
            top: '50%',
          }}
        ></motion.div>
