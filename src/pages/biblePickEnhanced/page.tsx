    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      <header className="px-1 pt-2">
        <button type="button" onClick={onReset} className="inline-flex items-center gap-2 text-sm font-semibold text-foreground-600">
          <i className="ri-arrow-left-line" aria-hidden="true" /> 다시 물어보기
        </button>
        <p className="mt-6 text-xs font-bold tracking-[0.16em] text-primary-600">AI 말씀 상담</p>
        <h1 className="mt-2 font-heading text-2xl font-bold leading-tight text-foreground-950 md:text-3xl">네 질문을 읽고<br />말씀으로 답해볼게요</h1>
      </header>

      <section className="rounded-card border border-background-200 bg-background-100 p-5 shadow-card md:p-7">
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-chip bg-primary-100 text-primary-700"><i className="ri-chat-3-line text-lg" aria-hidden="true" /></span>
          <div><p className="text-xs font-bold text-foreground-500">AI의 답</p><p className="mt-0.5 text-sm font-semibold text-foreground-800">감정 하나가 아니라 네 상황을 보고 답했어요</p></div>
        </div>
      ...
      <section className="rounded-card border border-background-200 bg-background-100 p-5 shadow-card md:p-6">
        <p className="text-xs font-bold tracking-wide text-primary-600">왜 이 말씀이 답이 될까요?</p>
        <p className="mt-3 text-[15px] leading-8 text-foreground-800">{result.recommendation}</p>
      </section>

      <section className="rounded-card border border-background-200 bg-background-100 p-5 shadow-card md:p-6">
        <div className="flex items-center gap-2"><i className="ri-footprint-line text-lg text-primary-600" aria-hidden="true" /><p className="text-sm font-bold text-foreground-900">오늘의 한 걸음</p></div>
      ...
      <section className="rounded-card border border-secondary-200 bg-secondary-50 p-5 md:p-6">
        <div className="flex items-center gap-2"><i className="ri-heart-3-line text-lg text-secondary-700" aria-hidden="true" /><p className="text-sm font-bold text-secondary-900">짧게 기도해볼까요?</p></div>
      ...
      <div className="grid grid-cols-2 gap-3 pb-2">
        <button type="button" onClick={onReset} className="min-h-12 rounded-input border border-background-300 bg-background-100 px-4 text-sm font-bold text-foreground-700 hover:bg-background-50">다른 질문하기</button>
        ...
        {result ? <ResultView result={result} onReset={() => { setResult(null); setError(''); }} /> : (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            <header className="mb-8 px-1 pt-2">
              ...
              <p className="mt-6 text-xs font-bold tracking-[0.16em] text-primary-600">GNH · AI 말씀 상담</p>
              <h1 className="mt-2 font-heading text-3xl font-bold tracking-tight text-foreground-950 md:text-4xl">궁금한 걸 그대로 물어보세요</h1>
              <p className="mt-4 max-w-lg text-[15px] leading-8 text-foreground-600">감정 하나를 고르는 대신, 지금 무슨 일이 있고 무엇이 고민되는지 적어주세요. 네 질문을 읽고 성경 말씀으로 답해볼게요.</p>
            </header>

            <form onSubmit={submit} className="rounded-card border border-background-200 bg-background-100 p-5 shadow-card md:p-7">
              <label htmlFor="bible-pick-question" className="mb-3 block text-sm font-bold text-foreground-900">무엇이 가장 고민되나요?</label>
              <textarea id="bible-pick-question" value={text} onChange={(event) => { setText(event.target.value); setError(''); }} placeholder="예) 친한 친구가 요즘 나를 피하는 것 같아요. 내가 뭘 잘못했는지 모르겠고, 먼저 물어봐도 될지 고민돼요." rows={7} maxLength={1000} className="w-full resize-none rounded-input border border-background-300 bg-background-50 px-4 py-4 text-[16px] leading-7 text-foreground-950 outline-none transition focus:border-primary-500 focus:ring-2 focus:ring-primary-100 dark:bg-background-100" />
              ...
              <button type="submit" disabled={!text.trim() || loading} className="mt-5 min-h-14 w-full rounded-input bg-primary-600 px-5 text-base font-bold text-background-50 shadow-card hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-40"><i className="ri-sparkling-2-line mr-2" aria-hidden="true" />{loading ? '네 질문을 읽고 답을 준비하고 있어요…' : '말씀으로 답받기'}</button>
            </form>

            <div className="mt-5 rounded-card border border-background-200 bg-background-100 p-4"><div className="flex items-start gap-3"><i className="ri-information-line mt-0.5 text-primary-600" aria-hidden="true" /><p className="text-xs leading-6 text-foreground-600">성경 말씀을 대신해 정답을 내려주는 기능은 아니에요. 네 상황을 말씀에 비추어 함께 생각할 수 있도록 돕는 응답이에요.</p></div></div>
          </motion.div>
        )}
        {loading && <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground-950/20 p-4" aria-live="polite"><div className="w-full max-w-sm rounded-card border border-background-200 bg-background-100 p-7 text-center shadow-card-lg"><div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-chip bg-primary-100 text-primary-700"><i className="ri-sparkling-2-line animate-pulse text-xl" aria-hidden="true" /></div><p className="font-bold text-foreground-950">네 이야기를 천천히 읽고 있어요</p><p className="mt-2 text-sm leading-6 text-foreground-600">질문의 핵심을 살펴보고, 그 상황에 맞는 말씀과 답을 준비하고 있어요.</p></div></div>}
      </div>
    </div>
  );
}
