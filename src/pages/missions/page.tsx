      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 md:py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-start justify-between gap-4 mb-6 md:mb-8">
            ...
            {canPublish && (
              <button type="button" onClick={() => setShowCreateForm(value => !value)} className="min-h-10 inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-primary-500 text-background-50 text-sm font-semibold hover:bg-primary-600 transition-colors cursor-pointer whitespace-nowrap">
          ...
          {error && <div className="mb-4 px-4 py-3 bg-accent-100 border border-accent-200 rounded-xl text-sm text-accent-700 flex items-center gap-2"><i className="ri-error-warning-line" />{error}</div>}
          {message && <div className="mb-4 px-4 py-3 bg-emerald-100 border border-emerald-200 rounded-xl text-sm text-emerald-700 flex items-center gap-2"><i className="ri-check-line" />{message}</div>}

          {canPublish && showCreateForm && (
            <motion.form initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleCreate} className="mb-6 bg-background-100 border border-background-200 rounded-2xl p-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground-950 mb-1.5">작은 사명 내용</label>
                <input value={newTitle} onChange={e => setNewTitle(e.target.value)} maxLength={100} required placeholder="예: 이번 주 금요일까지 교회 주변 쓰레기 줍기 할 사람?" className="w-full px-4 py-2.5 text-sm rounded-xl border border-background-200 bg-background-50 focus:border-primary-400 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground-950 mb-1.5">상세 설명</label>
                <textarea value={newDescription} onChange={e => setNewDescription(e.target.value)} rows={3} maxLength={500} placeholder="어디서, 언제까지, 무엇을 하면 되는지 적어주세요." className="w-full px-4 py-3 text-sm rounded-xl border border-background-200 bg-background-50 focus:border-primary-400 outline-none resize-none" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground-950 mb-1.5">카테고리</label>
                  <select value={newCategory} onChange={e => setNewCategory(e.target.value)} className="w-full px-4 py-2.5 text-sm rounded-xl border border-background-200 bg-background-50 focus:border-primary-400 outline-none appearance-none cursor-pointer">
                ...
                <div>
                  ...
                  <select value={newClub} onChange={e => setNewClub(e.target.value)} className="w-full px-4 py-2.5 text-sm rounded-xl border border-background-200 bg-background-50 focus:border-primary-400 outline-none appearance-none cursor-pointer">
                    <option value="">전체 (동아리 무관)</option>
                    {availableClubs.map(club => <option key={club} value={club}>{club}</option>)}
                  </select>
                </div>
              </div>
              <button type="submit" disabled={saving || !newTitle.trim()} className="min-h-10 px-5 py-2.5 rounded-full bg-primary-500 text-background-50 text-sm font-semibold hover:bg-primary-600 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">
          ...
          <div className="mb-5 flex items-center justify-between gap-3">
            ...
            <Link to="/missions/board" className="min-h-10 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-background-100 border border-background-200 text-xs font-semibold text-foreground-700 hover:bg-background-200 cursor-pointer whitespace-nowrap">
              <i className="ri-list-check-2" />내 작은 사명
            </Link>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {missions.length === 0 && <div className="col-span-full text-center py-12"><div className="w-14 h-14 rounded-2xl bg-background-200 flex items-center justify-center mx-auto mb-3"><i className="ri-checkbox-circle-line text-2xl text-foreground-400" /></div><p className="text-sm text-foreground-500">등록된 작은 사명이 없습니다</p></div>}
            {missions.map(mission => {
              ...
              return (
                <motion.article key={mission.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-background-100 border border-background-200 rounded-2xl p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-9 h-9 rounded-xl bg-accent-100 flex items-center justify-center flex-shrink-0"><i className={`${category.icon} text-accent-600`} /></div>
                    ...
                    {canDelete && <button type="button" onClick={() => void handleDelete(mission.id)} className="w-8 h-8 rounded-full flex items-center justify-center text-foreground-400 hover:text-rose-600 hover:bg-rose-50 cursor-pointer flex-shrink-0" title="작은 사명 삭제"><i className="ri-delete-bin-line text-xs" /></button>}
                  </div>
