...
export default function ScheduleEdit() {
  ...
  useEffect(() => {
    ...
    const fetchSchedule = async () => {
      try {
        ...
        setTargetClub(data.target_club || null);
      } catch {
  ...
  if (notFound) {
    return (
      <div className="min-h-screen bg-background-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-lg text-foreground-600 mb-4">일정을 찾을 수 없습니다</p>
          <button onClick={() => navigate('/schedule')} className="text-primary-600 hover:text-primary-700 font-medium cursor-pointer">일정 목록으로</button>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-8 md:py-12">
        ...
        >
          <div className="mb-8">
            ...
            <h1 className="text-2xl font-bold text-foreground-950 mb-1">일정 수정</h1>
            <p className="text-sm text-foreground-600">기존 일정을 수정합니다</p>
          </div>
