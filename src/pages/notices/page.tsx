...
export default function Notices(){
 const {user,profile}=useAuth(); const [notices,setNotices]=useState<NoticeItem[]>([]); const [readIds,setReadIds]=useState<Set<string>>(new Set()); const [loading,setLoading]=useState(true); const [error,setError]=useState<string|null>(null); const [selectedCategory,setSelectedCategory]=useState('일반');
 useEffect(()=>{let mounted=true;(async()=>{setLoading(true);setError(null);try{const {data,error}=await supabase.from('notices').select('id,author_id,author_name,title,content,category,is_pinned,created_at,updated_at').order('is_pinned',{ascending:false}).order('created_at',{ascending:false});if(error)throw error;const rows=(data||[]) as NoticeItem[];if(!mounted)return;setNotices(rows);const firstPopulated=NOTICE_CATEGORIES.find(category=>category!=='전체'&&rows.some(n=>(n.category||'일반')===category));if(firstPopulated)setSelectedCategory(firstPopulated);
