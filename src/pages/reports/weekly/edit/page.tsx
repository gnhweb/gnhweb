  ...
  useEffect(() => {
    ...
    const fetchReport = async () => {
      try {
        ...
        setAttendanceCount(String(data.attendance_count));
        setTotalMembers(String(data.total_members));
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 md:px-6 py-8 md:py-12">
      ...
      >
        <div className="mb-8">
          <button
            onClick={() => navigate(`/reports/weekly/${id}`)}
          ...
          <h1 className="text-2xl font-bold text-foreground-950 mb-1">주간 보고서 수정</h1>
        ...
        <div className="bg-background-100 border border-background-200 rounded-[20px] p-6 space-y-6">
          <div>
            ...
            <p className="text-xs text-foreground-600 mt-1.5">
              {weekStart ? getWeekRange(weekStart) : ''} / 월요일 기준
            </p>
          </div>
