  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-5 md:py-10 pb-28">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <div className="mb-5"><button onClick={() => navigate(`/reports/weekly/${id}`)} className="flex items-center gap-1.5 text-sm text-foreground-600 mb-3 cursor-pointer"><i className="ri-arrow-left-line" /> 보고서 상세로</button><h1 className="text-2xl font-bold text-foreground-950 mb-1">주간 보고서 수정</h1><p className="text-sm font-medium text-foreground-700">{clubLabel}</p></div>
        ...
        <div className="space-y-6">
          <section className="bg-background-100 border border-background-200 rounded-[20px] p-5 md:p-6 space-y-5">
            ...
            <div className="rounded-2xl bg-primary-50 border border-primary-100 p-4 flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-primary-800">전체 학생회원</p></div><strong className="text-2xl text-primary-700">{totalMembers}명</strong></div>
            <div className="flex items-center justify-between gap-3"><div><h2 className="font-bold">연습일</h2></div><input type="date" min={weekStart} max={weekEnd} onChange={(e) => { if (e.target.value) { addPracticeDay(e.target.value); e.currentTarget.value = ''; } }} className="px-3 py-2 rounded-xl border border-background-200 bg-background-50 text-xs" /></div>
          </section>
          ...
          {practiceEntries.map((entry, index) => (
            <section key={entry.practice_date} className="bg-background-100 border border-background-200 rounded-[20px] p-5 md:p-6 space-y-5">
              <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-primary-100 text-primary-700 flex items-center justify-center font-bold">{index + 1}</div><div><h2 className="font-bold">{formatPracticeDate(entry.practice_date)} 연습 기록</h2></div></div><button type="button" onClick={() => removePracticeDay(entry.practice_date)} className="text-xs font-semibold text-foreground-800 hover:text-accent-600 cursor-pointer">삭제</button></div>
              <div className="rounded-2xl bg-background-50 border border-background-200 p-4"><div className="flex justify-between mb-2"><label htmlFor={`we-attendance-${entry.practice_date}`} className="text-sm font-semibold">출석 인원</label><span className="text-xs text-foreground-700">전체 {totalMembers}명</span></div><input id={`we-attendance-${entry.practice_date}`} type="number" min="0" max={totalMembers || undefined} value={entry.attendance_count ?? ''} onChange={(e) => updatePracticeEntry(entry.practice_date, { attendance_count: e.target.value === '' ? null : Number(e.target.value) })} className="w-full px-4 py-3 rounded-xl border border-background-200 bg-background-100 text-sm" />{typeof entry.attendance_count === 'number' && totalMembers ? <p className="text-xs font-semibold text-primary-800 mt-2">출석률 {Math.round(entry.attendance_count / totalMembers * 100)}%</p> : null}</div>
              <div><label htmlFor={`we-progress-${entry.practice_date}`} className="block text-sm font-semibold mb-2">진행 상황 <span className="text-xs font-normal text-foreground-400">({entry.progress_summary.length}/500)</span></label><textarea id={`we-progress-${entry.practice_date}`} value={entry.progress_summary} onChange={(e) => updatePracticeEntry(entry.practice_date, { progress_summary: e.target.value })} maxLength={500} rows={4} className="w-full px-4 py-3 rounded-xl border border-background-200 text-sm resize-none bg-background-50" /></div>
              <div><label htmlFor={`we-notes-${entry.practice_date}`} className="block text-sm font-semibold mb-2">특이 사항 <span className="text-xs font-normal text-foreground-400">({entry.special_notes.length}/500)</span></label><textarea id={`we-notes-${entry.practice_date}`} value={entry.special_notes} onChange={(e) => updatePracticeEntry(entry.practice_date, { special_notes: e.target.value })} maxLength={500} rows={3} className="w-full px-4 py-3 rounded-xl border border-background-200 text-sm resize-none bg-background-50" /></div>
            </section>
          ))}

          <section className="bg-background-100 border border-background-200 rounded-[20px] p-5 md:p-6 space-y-5"><div className="grid grid-cols-2 md:grid-cols-3 gap-3"><div className="rounded-xl bg-primary-50 p-4"><p className="text-xs font-semibold text-primary-800">연습 횟수</p><p className="text-2xl font-black text-primary-700">{summary.practiceCount}회</p></div><div className="rounded-xl bg-secondary-50 p-4"><p className="text-xs font-semibold text-secondary-800">총 출석</p><p className="text-2xl font-black text-secondary-700">{summary.totalAttendance}명</p></div><div className="rounded-xl bg-background-50 border border-background-200 p-4"><p className="text-xs font-semibold text-foreground-800">평균 출석</p><p className="text-2xl font-black text-foreground-800">{summary.averageAttendance}명</p></div></div><div><label htmlFor="we-weekly-summary" className="block text-sm font-semibold mb-2">주간 총평 <span className="text-xs font-normal text-foreground-400">({progressSummary.length}/500)</span></label><textarea id="we-weekly-summary" value={progressSummary} onChange={(e) => setProgressSummary(e.target.value)} maxLength={500} rows={4} className="w-full px-4 py-3 rounded-xl border border-background-200 text-sm resize-none bg-background-50" /></div><div><label htmlFor="we-weekly-notes" className="block text-sm font-semibold mb-2">주간 총 특이사항 <span className="text-xs font-normal text-foreground-400">({specialNotes.length}/500)</span></label><textarea id="we-weekly-notes" value={specialNotes} onChange={(e) => setSpecialNotes(e.target.value)} maxLength={500} rows={3} className="w-full px-4 py-3 rounded-xl border border-background-200 text-sm resize-none bg-background-50" /></div><div className="flex flex-col sm:flex-row gap-3 pt-2"><button onClick={() => saveReport('draft')} disabled={saving} className="inline-flex justify-center items-center gap-1.5 px-5 py-2.5 rounded-full border border-background-200 text-sm font-medium cursor-pointer disabled:opacity-50"><i className="ri-save-line" />{saving ? '저장 중...' : '임시 저장'}</button><button onClick={() => saveReport('submitted')} disabled={saving} className="inline-flex justify-center items-center gap-1.5 px-5 py-2.5 rounded-full bg-primary-500 text-white text-sm font-medium cursor-pointer disabled:opacity-50"><i className="ri-send-plane-line" />{saving ? '저장 중...' : '수정 완료 및 제출'}</button></div></section>
        </div>
      </motion.div>
    </div>
  );
