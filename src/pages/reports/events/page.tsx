  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-8 md:py-12">
      ...
      >
        ...
        {canViewReports && (
          <div className="hidden md:flex items-center gap-1 bg-background-100 border border-background-200 rounded-full p-1 mb-6 overflow-x-auto">
            <button onClick={() => setClubFilter('all')} className={`px-3 py-1 rounded-full text-xs font-medium cursor-pointer whitespace-nowrap ${clubFilter === 'all' ? 'bg-primary-100 text-primary-700' : 'text-foreground-600'}`}>전체 동아리</button>
            {ALL_CLUBS.map(c => (
              <button key={c} onClick={() => setClubFilter(c)} className={`px-3 py-1 rounded-full text-xs font-medium cursor-pointer whitespace-nowrap ${clubFilter === c ? 'bg-primary-100 text-primary-700' : 'text-foreground-600'}`}>{CLUB_LABELS[c].split(' ')[0]}</button>
            ))}
          </div>
        )}
        ...
        <div className="md:hidden space-y-2 mb-6">
          ...
          {canViewReports && (
            <CategoryChipRow>
              ...
              {ALL_CLUBS.map(c => (
                <CategoryChip key={c} active={clubFilter === c} onClick={() => setClubFilter(c)}>{CLUB_LABELS[c].split(' ')[0]}</CategoryChip>
              ))}
            </CategoryChipRow>
          )}
        </div>
        ...
        {filteredReports.length === 0 ? (
          <div className="text-center py-20">
            ...
            {profile && profile.role !== 'member' ? (
              <Link to="/reports/events/write" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-primary-500 text-background-50 text-sm font-medium hover:bg-primary-600 transition-colors cursor-pointer whitespace-nowrap">
                <i className="ri-add-line"></i>첫 행사 보고서 작성하기
              </Link>
            ) : (
            ...
            <div className="hidden md:block space-y-3">
              {filteredReports.map((report, i) => (
                <motion.div key={report.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: i * 0.05 }}>
                  <Link to={`/reports/events/${report.id}`} className="block bg-background-100 border border-background-200 rounded-[20px] p-5 hover:border-background-300/60 transition-all duration-300 cursor-pointer">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          <div className="w-8 h-8 rounded-full bg-secondary-100 flex items-center justify-center"><i className="ri-calendar-event-line text-secondary-600 text-sm"></i></div>
                          <span className="text-sm font-semibold text-foreground-950 truncate">{report.event_name}</span>
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium shrink-0 ${EVENT_STATUS_COLORS[report.status]}`}>{EVENT_STATUS_LABELS[report.status]}</span>
                        </div>
                        <p className="text-sm text-foreground-700 line-clamp-2 mb-2">{report.performance_summary}</p>
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-foreground-600">
                          <span className="flex items-center gap-1"><i className="ri-calendar-line"></i>{formatDate(report.event_date)}</span>
                          ...
                          {report.participant_count > 0 && <span className="flex items-center gap-1"><i className="ri-team-line"></i>{report.participant_count}명 참여</span>}
                        </div>
                      </div>
              ...
              {filteredReports.map((report, i) => (
                <motion.div key={`m-${report.id}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: Math.min(i * 0.05, 0.3) }} whileTap={{ scale: 0.97 }}>
                  <Link to={`/reports/events/${report.id}`} className="relative flex gap-3 bg-background-100 border border-background-200 rounded-[20px] p-4 pl-3 overflow-hidden cursor-pointer">
                    ...
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-secondary-400 to-accent-400 flex items-center justify-center text-white flex-shrink-0 ml-1"><i className="ri-calendar-event-line text-sm"></i></div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-1"><span className="text-sm font-bold text-foreground-950 truncate">{report.event_name}</span><span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold ${EVENT_STATUS_COLORS[report.status]}`}>{EVENT_STATUS_LABELS[report.status]}</span></div>
                      <p className="text-xs text-foreground-600 line-clamp-1 mb-1.5">{report.performance_summary}</p>
                      <div className="flex items-center gap-2 text-[11px] text-foreground-500"><span>{formatDate(report.event_date)} · {report.author_name} · {CLUB_LABELS[report.club]}</span></div>
                    </div>
                  </Link>
                </motion.div>
              ))}
