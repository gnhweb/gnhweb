            ...
            {showForm && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-background-100 rounded-[20px] border border-gray-200 p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
                  <h2 className="text-lg font-bold text-foreground-950 mb-4">{editingId ? '말씀 수정' : '새 말씀 등록'}</h2>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {verses.map((verse, idx) => (
                ...
                >
                  <div
                    className="p-5 cursor-pointer focus-visible:ring-2 focus-visible:ring-secondary-400 focus-visible:outline-none rounded-[20px]"
                    role="button"
                    tabIndex={0}
                    aria-expanded={expandedVerse === verse.id}
                    onClick={() => setExpandedVerse(expandedVerse === verse.id ? null : verse.id)}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setExpandedVerse(expandedVerse === verse.id ? null : verse.id); } }}
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <h2 className="text-sm font-bold text-foreground-950">{verse.title}</h2>
