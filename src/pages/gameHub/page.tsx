...
// 게임 로직은 건드리지 않고 각 게임 라우트로 이동만 시키는 진입점 화면이다.
interface GameCard {
  path: string;
  title: string;
  tagline: string;
...
const GAMES: GameCard[] = [
  { path: '/wolves-and-sheep', title: '양과 늑대', tagline: '거짓 선지자를 찾아라!', icon: 'ri-user-3-line', gradient: 'from-rose-500 via-primary-500 to-accent-500', status: 'live' },
  { path: '/pharisee', title: '바리새인을 찾아라', tagline: '숨어있는 바리새인을 지목하라', icon: 'ri-search-eye-line', gradient: 'from-amber-500 via-orange-500 to-rose-500', status: 'live' },
  { path: '/galilee-phone', title: '갈릴리폰', tagline: '말씀을 전달해 완성하라', icon: 'ri-chat-smile-3-line', gradient: 'from-sky-500 via-secondary-500 to-primary-500', status: 'live' },
];
            ...
            {GAMES.map((game, i) => (
              ...
              >
                <button
                  ...
                  className="block w-full text-left touch-manipulation cursor-pointer"
                  aria-label={`${game.title} 시작`}
                >
                  <motion.div
                    whileTap={{ scale: 0.97 }}
                    transition={{ type: 'spring', stiffness: 500, damping: 25 }}
                    className={`relative aspect-[3/4] md:aspect-[16/10] rounded-[20px] overflow-hidden shadow-card bg-gradient-to-br ${game.gradient}`}
                  >
