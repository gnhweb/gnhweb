...
type Character = {
  name: string;
  traits: { courage: number; resilience: number; creativity: number; empathy: number };
  phrase: string;
  verse: string;
  desc: string;
  lesson: string;
...
function getRandomFallback(): MbtiResult {
  const character = CHARACTER_POOL[Math.floor(Math.random() * CHARACTER_POOL.length)];
  return {
    character: character.name,
    description: character.desc,
    lesson: character.lesson,
    matchingPhrase: character.phrase,
    bibleVerse: character.verse,
    traits: [
      { label: "용기", value: character.traits.courage },
      { label: "회복력", value: character.traits.resilience },
      { label: "창의성", value: character.traits.creativity },
      { label: "공감력", value: character.traits.empathy },
    ],
    bestWith: "요셉",
...
function koreanOnly(value: unknown): boolean {
  return typeof value === "string" && !/[A-Za-z]/.test(value) && !/[\u4E00-\u9FFF]/.test(value) && value.trim().length > 0;
}

function validateTraits(value: unknown): value is Trait[] {
  return Array.isArray(value)
    && value.length >= 4
    && value.every((item) => {
      if (!item || typeof item !== "object") return false;
      const trait = item as Trait;
      return typeof trait.label === "string"
        && trait.label.trim().length > 0
        && Number.isFinite(trait.value)
        && trait.value >= 0
        && trait.value <= 100;
    });
}
...
function validateKoreanFields(parsed: Record<string, unknown>): boolean {
  return ["description", "lesson", "bibleVerse", "matchingPhrase", "challenge", "bestWith"]
    .every((field) => koreanOnly(parsed[field]));
}
...
export async function handleNimMbti(req: Request, env: Record<string, string | undefined>): Promise<Response> {
  ...
  try {
    ...
    if (answers.length < 4) return new Response(JSON.stringify(getRandomFallback()), { headers: CORS_HEADERS });

    const shuffledPool = [...CHARACTER_POOL].sort(() => Math.random() - 0.5);
    const suggestedChars = shuffledPool.slice(0, 8).map((character, index) =>
      `${index + 1}. ${character.name}: 용기 ${character.traits.courage} / 회복력 ${character.traits.resilience} / 창의성 ${character.traits.creativity} / 공감력 ${character.traits.empathy}`,
    ).join("\n");
    const randomSeed = Math.floor(Math.random() * 100000);

    const gatewayUrl = (env.CLOUDFLARE_AI_GATEWAY_URL || "https://gnhweb-ai-gateway.gemini19840314.workers.dev").replace(/\/$/, "");
    ...
    const answersText = answers.map((answer, index) => `${index + 1}번: ${String(answer)}`).join("\n");
    ...
    if (!response.ok) return new Response(JSON.stringify(getRandomFallback()), { headers: CORS_HEADERS });

    const payload = await response.json() as { choices?: Array<{ message?: { content?: unknown } }> };
    const content = payload.choices?.[0]?.message?.content;
    if (typeof content !== "string" || !content) return new Response(JSON.stringify(getRandomFallback()), { headers: CORS_HEADERS });

    const parsed = parseJson(content);
    if (!parsed || !parsed.character || !parsed.description) return new Response(JSON.stringify(getRandomFallback()), { headers: CORS_HEADERS });

    if (!validateKoreanFields(parsed)) return new Response(JSON.stringify(getRandomFallback()), { headers: CORS_HEADERS });

    if (!validateTraits(parsed.traits)) {
      const fallback = getRandomFallback();
      parsed.traits = fallback.traits;
    }

    const characterName = String(parsed.character || "");
    if (!CHARACTER_POOL.some((character) => character.name === characterName) && characterName.length > 0) {
      const fallback = getRandomFallback();
      parsed.character = fallback.character;
      parsed.description = fallback.description;
      parsed.lesson = fallback.lesson;
    }

    return new Response(JSON.stringify(parsed), { headers: CORS_HEADERS });
  } catch (error) {
    console.error("[nim-mbti]", error);
    return new Response(JSON.stringify(getRandomFallback()), { headers: CORS_HEADERS });
  }
}
