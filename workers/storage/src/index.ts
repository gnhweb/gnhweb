...
if (request.method === 'POST') {
  ...
  const userId = await getCanonicalUserId(authorization);
  ...
  if (action === 'upload') {
    ...
    if (!canWriteMemory && !canWriteMissionProof && !canWriteAvatar && !canWriteOperational && !canWriteClubPhoto) {
      if (postStorageKey.startsWith('memories/')) {
        ...
        return json({
          error: `Forbidden: memory owner mismatch (path=${owner.slice(-8)}, canonical=${userId.slice(-8)})`,
        }, 403, cors);
      }
